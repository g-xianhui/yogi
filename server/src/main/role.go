...
func RoleLogin(conn net.Conn) (*User, error) {
	logger.Println("RoleLogin")
	buf := make([]byte, 1024)

	req := &msg.MQLoginChallenge{}
	if err := ReadPackage(conn, buf, req); err != nil {
		return nil, err
	}
	...
	NetSend(conn, 0, msg.MRLOGINCHALLENGE, challenge_rep, nil)
	...
	if err := ReadPackage(conn, buf, verify_req); err != nil {
		return nil, err
	}
	...
	NetSend(conn, 0, msg.MRLOGINVERIFY, verify_rep, nil)
	logger.Printf("verify success: %d", account.guid)

	// login result
	user, err := theUserMgr.Load(account.guid)
	...
	NetSend(conn, seq_num, msg.MRLOGINRESULT, result_rep, skey)
...
func RoleQuit(user *User) {
	logger.Printf("user[%d] quit", user.GetId())
}

func RoleKick(user *User, msg interface{}) {
...
func init() {
	theMsgMgr.Register(msg.FMKick, RoleKick)
}
